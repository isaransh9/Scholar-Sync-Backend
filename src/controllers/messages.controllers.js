...
const getOneUserConversation = asyncHandler(async (req, res) => {
  ...
  if (!conversation) {
    return res
      .status(200)
      .json(new ApiResponse(200, [], "These users have no chat history!!"));
  }
...
const getAllUserData = asyncHandler(async (req, res) => {
  ...
  const myId = req.user._id;
  // console.log(myId);
  const userData = await User.find({ _id: { $ne: myId } }).select(
    "-password -refreshToken"
  );
  return res
    .status(200)
    .json(new ApiResponse(200, userData, "Data fetched successfully"));
});
